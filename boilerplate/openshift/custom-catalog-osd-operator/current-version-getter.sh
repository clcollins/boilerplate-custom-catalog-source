...
function push_version_file_changes() {
  ...
  if [[ -n $(git diff --name-only -q ${VERSIONS_DIR}) ]]; then
    ...
    if [[ -z "${APP_SRE_BOT_EMAIL}" || -z "${APP_SRE_BOT_USER}" || -z "${APP_SRE_BOT_PUSH_TOKEN}" ]]; then
      echo "APP_SRE_BOT_EMAIL or APP_SRE_BOT_USER or APP_SRE_BOT_PUSH_TOKEN env variables not set to perform git push... Abort"
      exit 1
    fi
    # set up app-sre git bot to 'git push'
    export GIT_TOKEN=${APP_SRE_BOT_PUSH_TOKEN}
    git config user.email "${APP_SRE_BOT_EMAIL}"
    git config user.name "${APP_SRE_BOT_USER}"
...
# if -p is set, commit and push
if [[ ${git_push} == "true" ]]; then
  push_version_file_changes
fi
